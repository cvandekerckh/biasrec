...
def assign_users_by_quota(users, condition_quotas, seed=42):
    """
    Randomly assign users to conditions following target quotas.

    Args:
        users (list): list of user_ids
        condition_quotas (dict): {condition_id: proportion}
        seed (int): random seed for reproducibility

    Returns:
        dict: {user_id: condition_id}
    """
    rd.seed(seed)
    users = users.copy()
    rd.shuffle(users)
    ...
    # Step 1 — raw target sizes
    raw_sizes = {
        cid: quota * N
        for cid, quota in condition_quotas.items()
    }

    # Step 2 — integer sizes (floor)
    sizes = {
        cid: int(np.floor(size))
        for cid, size in raw_sizes.items()
    }

    # Step 3 — distribute remaining users (largest remainder method)
    remaining = N - sum(sizes.values())
    ...
    for cid, _ in remainders[:remaining]:
        sizes[cid] += 1
...
def create_recommendations():
    ...
    with app.app_context():
        ...
        user_conditions = assign_users_by_quota(
            eligible_users,
            CONDITION_QUOTAS,
            seed=42
        )
        ...
        cnt = Counter(user_conditions.values())
        total = sum(cnt.values())

        print("\n=== Condition distribution (quota check) ===")
        for cid in sorted(CONDITION_QUOTAS):
            observed = cnt.get(cid, 0) / total
            target = CONDITION_QUOTAS[cid]
            print(
                f"Condition {cid}: "
                f"{cnt.get(cid, 0)} users "
                f"({observed:.2%} | target={target:.2%})"
            )
        
        assert set(user_conditions.keys()) == set(eligible_users), \
            "Some eligible users were not assigned to a condition"

        assert total == len(eligible_users), \
            "Mismatch between assigned users and eligible users"
        ...
        df_users_by_error.to_csv(
            Cf.DATA_PATH_OUT / "users_by_error_bin_and_condition_25_01_2026_test1.csv",
            index=False
        )    
...
if __name__ == "__main__":
    ...
    with open(Cf.DATA_PATH_OUT / "biased_recommendations_test_prolific_25_01_2026_final.p", "wb") as f:
        pickle.dump(recs, f)
